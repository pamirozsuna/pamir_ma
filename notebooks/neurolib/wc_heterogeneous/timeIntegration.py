...
def timeIntegration(params):
    ...
    # check 
    if len(c_excexc) != N or len(c_excinh) != N or len(c_inhexc) != N or len(c_inhinh) != N or len(sigma_ou) != N or len(exc_ext) != N or len(inh_ext) != N :
        ...
        for no in range(N):
            ...
            exc_rhs = (
                ...
                * (
                    ...
                    + (1 - excs[no, i - 1])
                    * S_E(
                        c_excexc[no] * excs[no, i - 1]  # input from within the excitatory population
                        - c_inhexc[no] * inhs[no, i - 1]  # input from the inhibitory population
            ...
            inh_rhs = (
                ...
                * (
                    ...
                    + (1 - inhs[no, i - 1])
                    * S_I(
                        c_excinh[no] * excs[no, i - 1]  # input from the excitatory population
                        - c_inhinh[no] * inhs[no, i - 1]  # input from within the inhibitory population
                        + inh_ext[no]
                    )  # external input
                    + inh_ou[no]  # ou noise
                )
            ...
            exc_ou[no] = (
                exc_ou[no] + (exc_ou_mean - exc_ou[no]) * dt / tau_ou + sigma_ou[no] * sqrt_dt * noise_exc[no]
            )  # mV/ms
            inh_ou[no] = (
                inh_ou[no] + (inh_ou_mean - inh_ou[no]) * dt / tau_ou + sigma_ou[no] * sqrt_dt * noise_inh[no]
            )  # mV/ms
