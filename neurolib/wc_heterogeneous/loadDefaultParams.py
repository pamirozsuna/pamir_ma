...
def loadDefaultParams(Cmat=None, Dmat=None, seed=None):
    ...
    params.sigma_ou = np.full(params.N, 0.0)   # noise intensity
    ...
    params.c_excexc = np.full(params.N, 16)   # local E-E coupling
    params.c_excinh = np.full(params.N, 15)   # local E-I coupling
    params.c_inhexc = np.full(params.N, 12)   # local I-E coupling
    params.c_inhinh = np.full(params.N, 3)   # local I-I coupling
    ...
    params.exc_ext = np.full(params.N, 0)  # baseline external input to E
    params.inh_ext = np.full(params.N, 0)   # baseline external input to I
